...
//     localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(columnConfig));
// }
function saveColumnConfig() {
...
//     // Update the `visible` property in columnConfig based on savedColumns
//     columnConfig.forEach((column) => {
//       column.visible = savedColumns.length === 0 || savedColumns.includes(column.id);
//     });

//     // Regenerate column options and re-render the table
...
//     console.log(`SMA Applied with time period: ${smaTimePeriod} and time interval: ${smaTimeInterval}`);
...
//         // Check if the SMA column already exists in the columnConfig
//         console.log('the column config from apply sma: ', columnConfig)
//         const existingColumn = columnConfig.find(col => col.id === smaColumnId);
//         if (!existingColumn) {
//             // Add new SMA column to columnConfig
//             columnConfig.push({
//                 id: smaColumnId,
//                 label: `SMA ${smaTimePeriod}`,
//                 visible: true, // Make the SMA column visible
//                 editable: false // Optional: make SMA column non-editable
//             });
...
//         smaData.forEach((smaRecord) => {
//             // Find the row by symbol_code and update the SMA value
//             const row = document.querySelector(`tr[data-symbol="${smaRecord.symbol_code}"]`);
//             if (row) {
//                 let smaCell = row.querySelector(`td.${smaColumnId}`);
//                 console.log(smaCell)
//                 if (smaCell) {
//                     smaCell.textContent = smaRecord.sma;
//                 } else if(smaCell == null){
//                     console.log('creating')
//                     // If SMA cell doesn't exist, create one
//                     smaCell = document.createElement("td");
//                     smaCell.classList.add(smaColumnId);
//                     smaCell.textContent = smaRecord[`sma${smaTimePeriod}`];
//                     row.appendChild(smaCell);
//                 }
//             }
//         });
//     })
...
//     const rsiColumnId = `rsi_${rsiInterval}`; // Unique ID for RSI column
...
//         // Add RSI column to `columnConfig` if it doesn't exist
//         if (!columnConfig.some((col) => col.id === rsiColumnId)) {
//           columnConfig.push({
//             id: rsiColumnId,
//             label: `RSI ${rsiInterval}`,
//             visible: true,
//             editable: false,
//           });
//           saveColumnConfig(); // Persist changes
...
//         // Update RSI values for all rows
//         updateTechnicalData(rsiData, rsiColumnId, `rsi${rsiInterval}`);
//       })
//       .catch((error) => {
//         console.error('Error applying RSI:', error);
